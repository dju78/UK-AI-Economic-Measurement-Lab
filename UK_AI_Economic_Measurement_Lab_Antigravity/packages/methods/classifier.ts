...
const HARD_NEGATIVE_PATTERNS = [
  ...
  /\b((using|uses|utilises?|leveraging)\s+(an?\s+)?(chatgpt|ai|generative ai|copilot|accounting package)|ai[\s\-]ready\s+(cloud\s+hosting|colocation)|reselling\s+(generic\s+)?cloud\s+hosting|smart\s+technology\s+and\s+toner)\b/i
];
